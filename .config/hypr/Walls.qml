...
Item {
  ...
  // ─── Matugen: una sola invocación ──────────────────────────────
  // Antes se llamaba matugen dos veces (una para aplicar templates,
  // otra con --json para leer colores) + un proceso python3 para
  // limpiar el JSON. Ahora una sola pasada: --quiet silencia los
  // logs para que stdout quede como JSON puro, y sigue aplicando
  // los templates configurados en matugen/config.toml igual que
  // antes. Esto ahorra 2 procesos por cada cambio de wallpaper.
  Process {
    id: matugenProc
    command: ["bash", "-c",
      "matugen image " + wallsRoot.shQuote(applyWallpaper.wpPath) +
      " --prefer lightness -t scheme-" + wallsRoot.schemes[wallsRoot.schemeIndex].name +
      " --quiet --json hex > /tmp/matugen-colors.json 2>/dev/null"
    ]
    running: false
    ...
    Rectangle {
      ...
      Rectangle {
        ...
        RowLayout {
          ...
          spacing: 0
          Row {
            id: paletteDotsRow; spacing: 5; Layout.alignment: Qt.AlignVCenter
            Repeater {
              model: wallsRoot.paletteSlots
              delegate: Item {
                width: 20; height: 20
                Rectangle {
                  anchors.centerIn: parent
                  width: index === wallsRoot.schemeIndex ? 16 : 10; height: index === wallsRoot.schemeIndex ? 16 : 10; radius: width / 2
                  color: modelData.color ?? "#888"; border.color: index === wallsRoot.schemeIndex ? "#ffffff" : Qt.rgba(1,1,1,0.18); border.width: index === wallsRoot.schemeIndex ? 2 : 1
                  Behavior on width { NumberAnimation { duration: 200 } }
                  Behavior on height { NumberAnimation { duration: 200 } }
                }
                MouseArea { anchors.fill: parent; onClicked: wallsRoot.applyScheme(index); cursorShape: Qt.PointingHandCursor }
              }
            }
          }
          Item { Layout.fillWidth: true }
          Text { text: wallsRoot.schemes[wallsRoot.schemeIndex].label; color: wallsRoot.matugenColors.accent; font.pixelSize: 9; font.family: "JetBrainsMono Nerd Font" }
        }
      }
